...
export default function SearchedRides() {
    ...
    const CaptureSelectionInfo = (e) => {
        console.log(BookingFareDetails[e]);
        console.log(tripDetails);
        localStorage.setItem("RidesSearched", 'Yes');
        localStorage.setItem("BookingData", JSON.stringify({ Fare: (BookingFareDetails[e]), Trip: tripDetails }));
                                    ...
                                    {BookingFareDetails?.map((e, index) => (
                                        <tr key={index} >
                                            ...
                                            <td className="  place-items-center text-center"> <div className="w-full flex flex-col items-center justify-center">
                                                ...
                                                <button
                                                    className="shadow-md px-4 py-2 mt-2 cursor-pointer bg-[--c1] capitalize transition-colors duration-300 transform bg-green-300 text-black rounded-md text-2xl font-semibold hover:bg-green-400 hover:scale-95 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
                                                    onClick={() => CaptureSelectionInfo(index)} // Replace 0 with your index logic
                                                    disabled={Loading} // Disable button when loading
                                                >
                                                    {Loading ? (
                                                        <>
                                                            <FaSpinner className="animate-spin mr-2 text-sm " /> {/* Spinner with animation */}
